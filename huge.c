...
static void *huge_chunk_alloc(struct thread_cache *cache,
                              size_t size,
                              size_t alignment,
                              struct arena **out_arena)
{
    ...
    if (chunk) {
        ...
        // Work around the possibility of holes created by huge_move_expand (see
        // below).
...
static void *huge_move_expand(struct thread_cache *cache,
                              void *old_addr,
                              size_t old_size,
                              size_t new_size)
{
    ...
    if (unlikely(memory_remap_fixed(old_addr, old_size, new_addr, new_size))) {
        ...
        // Attempt to fill the virtual memory hole. The kernel should provide a
        // flag for preserving the old mapping to avoid the possibility of this
        // failing and creating fragmentation.
        //
...
void *huge_realloc(struct thread_cache *cache,
                   void *ptr,
                   size_t old_size,
