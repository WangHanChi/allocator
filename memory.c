...
{
    int overcommit =
        open("/proc/sys/vm/overcommit_memory", O_RDONLY | O_CLOEXEC);
    if (overcommit != -1) {
        char digit;
...
{
    int prot =
        !commit && reduce_commit_charge ? PROT_NONE : PROT_READ | PROT_WRITE;
...
void *memory_map_aligned(void *hint, size_t size, size_t alignment, bool commit)
{
    ...
    size_t lead_size =
        ALIGNMENT_CEILING((uintptr_t) addr, alignment) - (uintptr_t) addr;
...
bool memory_remap_fixed(void *addr,
                        size_t old_size,
                        void *new_addr,
                        size_t new_size)
{
    return mremap(addr, old_size, new_size, MREMAP_MAYMOVE | MREMAP_FIXED,
                  new_addr) == MAP_FAILED;
}
